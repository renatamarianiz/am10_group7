...
analysis_1 <- brazil_cities %>% 
  drop_na(weekly_cases_per_100k) %>% 
  group_by(name) %>% 
  summarise(max_cases_per_100k = max(weekly_cases_per_100k), population = mean(population), capital = mean(capital)) %>% 
...
```{r}
library(zoo)
ggplot(brazil_cities) + geom_boxplot(aes(x=max_class, y=weekly_cases_per_100k), alpha=0.02)

classes_over_time <- brazil_cities %>% 
  drop_na(weekly_cases_per_100k, max_class) %>% 
  select(max_class, weekly_cases_per_100k, date) %>% 
  group_by(date, max_class) %>% 
  summarise(avg_cases_100k = mean(weekly_cases_per_100k)) %>% 
